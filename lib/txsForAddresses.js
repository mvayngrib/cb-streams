...
module.exports = function txstream (options) {
  ...
  stream.resume = function () {
    isPaused = false
    if (unprocessedTxs) {
      onFetched(null, unprocessedTxs)
      unprocessedTxs = null
    } else if (fetchOnResumed) {
      fetch()
    }
  }
  ...
  function fetch () {
    ...
    debug('fetching txs')
    fetchOnResumed = false
    blockchain.addresses.transactions(options.addresses, lastHeight, onFetched)
