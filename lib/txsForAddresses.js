    ...
    blockchain.addresses.transactions(options.addresses, lastHeight, function (err, txs) {
      ...
      if (live) {
        ...
        txs = txs.filter(function (tx) {
          if (typeof tx.blockHeight !== 'undefined' && tx.blockHeight < lastHeight) {
            return
          }
        ...
        var maxHeightFilter = atMaxHeight.bind(null, maxHeight)
        lastBlock = lastBlock.filter(maxHeightFilter)

        txs.filter(atMaxHeight)
          .forEach(lastBlock.push, lastBlock)

        lastHeight = Math.max(lastHeight, maxHeight)
